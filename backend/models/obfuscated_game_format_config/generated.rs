...
impl ObfuscatedGameFormatConfig {
    pub fn create(
        db: &mut Connection,
        item: &CreateObfuscatedGameFormatConfig,
    ) -> QueryResult<Self> {
        use crate::schema::obfuscated_game_format_config::dsl::*;

        insert_into(obfuscated_game_format_config)
            .values(item)
            .get_result::<Self>(db)
    }
    ...
    pub fn paginate(
        ...
        let items = obfuscated_game_format_config
            .limit(page_size)
    ...
    pub fn update(
        ...
        item: &UpdateObfuscatedGameFormatConfig,
    ) -> QueryResult<Self> {
        use crate::schema::obfuscated_game_format_config::dsl::*;

        diesel::update(obfuscated_game_format_config.filter(repository.eq(param_repository)))
            .set(item)
            .get_result(db)
    }
    ...
    pub fn delete(db: &mut Connection, param_repository: String) -> QueryResult<usize> {
        ...
        diesel::delete(obfuscated_game_format_config.filter(repository.eq(param_repository)))
            .execute(db)
    }
}
