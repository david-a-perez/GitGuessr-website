...
#[diesel(table_name=obfuscated_game_format_config, primary_key(repository), belongs_to(Repository, foreign_key=repository))]
...
impl ObfuscatedGameFormatConfig {
    ...
    pub fn read(db: &mut Connection, param_repository: String) -> QueryResult<Self> {
        ...
        obfuscated_game_format_config.filter(repository.eq(param_repository)).first::<Self>(db)
    }
    ...
    pub fn update(db: &mut Connection, param_repository: String, item: &UpdateObfuscatedGameFormatConfig) -> QueryResult<Self> {
        use crate::schema::obfuscated_game_format_config::dsl::*;

        diesel::update(obfuscated_game_format_config.filter(repository.eq(param_repository))).set(item).get_result(db)
    }

    pub fn delete(db: &mut Connection, param_repository: String) -> QueryResult<usize> {
        use crate::schema::obfuscated_game_format_config::dsl::*;

        diesel::delete(obfuscated_game_format_config.filter(repository.eq(param_repository))).execute(db)
    }
