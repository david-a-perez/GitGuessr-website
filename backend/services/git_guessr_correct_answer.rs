use crate::models::git_guessr_correct_answer::GitGuessrCorrectAnswer;
use actix_web::{
    error::ErrorNotFound,
    get,
    web::{Data, Path, Query},
...
async fn index(db: Data<Database>, Query(info): Query<PaginationParams>) -> impl Responder {
    actix_web::web::block(move || {
        ...
        GitGuessrCorrectAnswer::paginate(&mut conn, info.page, info.page_size)
    })
    .await
...
async fn read(db: Data<Database>, item_id: Path<i32>) -> impl Responder {
    actix_web::web::block(move || {
        ...
        GitGuessrCorrectAnswer::read(&mut conn, item_id.into_inner())
    })
    .await
    .map(|result| match result {
        Ok(result) => Ok(HttpResponse::Ok().json(result)),
...
//         GitGuessrCorrectAnswer::create(&mut conn, &item)
//     })
//     .await
//     .map(|result| match result {
//         Ok(result) => Ok(HttpResponse::Ok().json(result)),
...
//         GitGuessrCorrectAnswer::update(&mut conn, item_id.into_inner(), &item)
//     })
//     .await
//     .map(|result| match result {
//         Ok(result) => Ok(HttpResponse::Ok().json(result)),
...
//         GitGuessrCorrectAnswer::delete(&mut conn, item_id.into_inner())
//     })
//     .await
//     .map(|result| match result {
//         Ok(result) => Ok(HttpResponse::Ok().json(result)),
