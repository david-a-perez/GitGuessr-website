...
async fn create(
    db: Data<Database>,
    Json(item): Json<CreateCorrectAnswer>,
) -> Result<HttpResponse, Error> {
    let mut con = db.get_connection();

    let result = CorrectAnswer::create(&mut con, &item).expect("Creation error");
...
async fn update(
    ...
    let result = CorrectAnswer::update(&mut con, item_id.into_inner(), &item);
